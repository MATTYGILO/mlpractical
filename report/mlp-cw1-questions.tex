...
\youranswer{the model with 128 hidden units overfits. This can be seen clearly in Figure 2a, as it's validation error increases
...
accuracy and increases the validation loss. Therefore, our model which overfits the least is the width size 64. This
observation is supported by literature, where if the model's width is too small it's model capacity cannot effectively
model the signal of the data, and if it's width is too large it's capacity is too large and begins to overfit
the data's noise.
...
method there is a point that once cross degrades performace significiantly. From our results, we can see that \_ with a
parameters value of \_ provides the greatest validation accuracy of \_. Additionally, we can see the method that has
overfit the least is \_, with a generalisation gap of \_ - \_ = \_. Comparing these methods to the baseline and soft
labelled baseline we can conclude that \_. Comparing L1 and L2 regularisation, we can conclude for the parameters tested
\_ is a better regularisation method for accuracy and \_ is the least overfit regularisation method. Overall, we can
conclude that overfitting is being tackled by \_. Furthermore, we can conclude that ther \_, between validation accuracy
and generalisation gap, this suggests that \_. Overall, our best performing model is \_. (Citation about this)




From a gen
as it provides greatest validation accuracy and lowest validation

For each regularisation
 method, we can see that beyond a point increasing it's parameter size can hamper
...
\youranswer{Overall we can conclude that \_. Our results are generally supported by the surrounding literature, \_.
...
\begin{table*}[t]
    ...
    \midrule
            Label smoothing & 0.747   &   1.352   &   1.508   \\
    \bottomrule
    \end{tabular}
