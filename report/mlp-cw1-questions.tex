...
The slight rise in the generalisation gap from depth two to depth three suggests the early onset of overfitting: the model is becoming better at fitting the training data without achieving corresponding gains on unseen data. This behaviour is consistent with standard theory and prior literature, which show that deeper networks can represent more complex functions but are also more prone to fitting noise when the task does not require the additional capacity.
Overall, the results align with expectations: moderate increases in depth can improve performance, but beyond a certain point the added complexity offers diminishing returns and begins to harm generalisation.
}
}
