...
\youranswer{Due to limit experimental budget we tested 4 combinations of L1L2 parameters. For both regularisers we found parameters in the range 1e-13 to 1e-9 were most effective.
    We tried 4 combinations of parameters for the combined L1L2 regularisation (l1: 1e-11, l2: 1e-11) a balanced small regularisation both in effective ranges.
Next we tried (l1: 1e-9, l2: 1e-9) for a balanced moderate penalty.
Then we tried a stronger l1 (l1: 1e-9, l2: 1e-11) to see if sparsity improved added benefits.
Finally we tried a stronger l2 (l1: 1e-11, l2: 1e-9) to see if weight decay improved generalisation.
...
The four combinations I would


(Even if you have not managed to implement your L1 and L2 combination; still argue based on your proposed approach).


The problem with the above assumption, is that we only know the effect of changing the parameters individually and not the combinatorics of the \textbf{parameters}.
By combining the regularisors it could lead to overregularisation and hinder performance.
